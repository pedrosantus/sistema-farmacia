...
import { deletarUsuarioAction, Usuario } from "@/actions/usuario"

interface PopupDeleteUsuarioProps {
    usuario: Usuario
    onClose?: () => void
    onSucesso?: (id: string) => void
}

export default function PopupDeleteUsuario({ usuario, onClose, onSucesso }: PopupDeleteUsuarioProps) {
        ...
        <form onSubmit={handleSubmit} className="bg-white grid gap-y-5">
            <div className="flex flex-col gap-2 pb-2 border-b border-gray-100">
                ...
                <p className="text-sm text-gray-500">
                    Tem certeza que deseja remover <strong>{usuario.nome}</strong>?
            ...
            {sucesso && (
                <div className="p-2 rounded-md bg-green-50 border border-green-200">
                    <span className="text-green-700 text-sm font-medium">✓ Usuário removido com sucesso!</span>
                </div>
            )}
